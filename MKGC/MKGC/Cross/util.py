...
def load_training_triples(triple_file):
    ...
    for line in lines:
        ...
        tail = line_arr[1]
        rel = line_arr[2]
        # head = str(line_arr[0]).encode("utf8")
        # tail = str(line_arr[1]).encode("utf8")
        # rel = str(line_arr[2]).encode("utf8")
...
def load_entity_list(triple_file, entity_embeddings_structure, entity_embeddings_description, entity_embeddings_image, entity_embeddings_multi):
    entity_list = []
    triples = []
    ...
    for line in lines:
        ...
        if head in entity_embeddings_structure and tail in entity_embeddings_structure and head in \
                entity_embeddings_description and tail in entity_embeddings_description and \
                head in entity_embeddings_image and tail in entity_embeddings_image and head in entity_embeddings_multi \
                and tail in entity_embeddings_multi:
            entity_list.append(head)
            entity_list.append(tail)
            triples.append(line)
...
def load_binary_file(in_file, py_version=2):
    if py_version == 2:
        with open(in_file, 'rb') as f:
            embeddings = pickle.load(f, encoding='utf8')
            return embeddings
    else:
        with open(in_file, 'rb') as f:
...
def load_freebase_triple_data_multimodal(train_triples_file, entity_embeddings_structural, entity_embeddings_description,
                                         entity_embeddings_image, entity_embeddings_multimodal, relation_embeddings):
    training_intances = []
    ...
    for triple in training_triples:
        ...
        tail = triple[1]
        rel = triple[2]
        if head in entity_embeddings_structural and tail in entity_embeddings_structural and \
                head in entity_embeddings_description and tail in entity_embeddings_description and \
                head in entity_embeddings_image and tail in entity_embeddings_image and head in \
                entity_embeddings_multimodal and tail in entity_embeddings_multimodal:
            head_embd_structural = entity_embeddings_structural[head]#/ np.linalg.norm(entity_embeddings_txt[head])
            tail_embd_structural = entity_embeddings_structural[tail]#/ np.linalg.norm( entity_embeddings_txt[tail])
            head_embd_description = entity_embeddings_description[head]
            tail_embd_description = entity_embeddings_description[tail]
            head_embd_image = entity_embeddings_image[head]#/ np.linalg.norm(entity_embeddings_img[head])
            tail_embd_image = entity_embeddings_image[tail]#/ np.linalg.norm( entity_embeddings_img[tail])
            head_embd_multimodal = entity_embeddings_multimodal[head]
            tail_embd_multimodal = entity_embeddings_multimodal[tail]
            rel_embd = relation_embeddings[rel]#/ np.linalg.norm( relation_embeddings[rel])
            train_instance = (head_embd_structural, rel_embd, tail_embd_structural, head_embd_description,
                              tail_embd_description, head_embd_image, tail_embd_image, head_embd_multimodal,
                              tail_embd_multimodal, head, rel, tail)
...
def get_batch_with_neg_heads_and_neg_tails_multimodal(training_data, triples_set, entity_list, start, end,
                                                      entity_embedding_structure, entity_embedding_description,
                                                      entity_embedding_image, entity_embedding_multi):

    h_data_structure = []  # head structure embeddings
    h_data_description = [] # head description embeddings
    h_data_image = []  # head image embeddings
    h_data_multi = []
    h_neg_data_structure = []  # head structure embeddings
    h_neg_data_description = [] # head description embeddings
    h_neg_data_image = []  # head image embeddings
    h_neg_data_multi = []
    r_data = []  # relation embeddings
    t_data_structure = []  # tail structure embeddings
    t_data_description = [] # tail description embeddings
    t_data_image = []  # tail image embeddings
    t_data_multi = []
    t_neg_data_structure = []  # negative tail structure embeddings
    t_neg_data_description = [] # negative tail description embeddings
    t_neg_data_image = []  # negative tail image embeddings
    t_neg_data_multi = []
    ...
    for triple in batch_data:
        ...
        # train_instance = (head_embd_strucure, rel_embd, tail_embd_structure,
        # head_embd_description, tail_embd_description, head_embd_image, tail_embd_image, head, rel, tail)

        h_data_structure.append(triple[0])
        r_data.append(triple[1])
        t_data_structure.append(triple[2])
        h_data_description.append(triple[3])
        t_data_description.append(triple[4])
        h_data_image.append(triple[5])
        t_data_image.append(triple[6])
        h_data_multi.append(triple[7])
        t_data_multi.append(triple[8])

        text_triple = (triple[9], triple[10], triple[11])
        ...
        t_neg_embed_structure = entity_embedding_structure[t_neg]
        t_neg_embed_description = entity_embedding_description[t_neg]
        t_neg_embed_image = entity_embedding_image[t_neg]
        t_neg_embed_multi = entity_embedding_multi[t_neg]
        t_neg_data_structure.append(t_neg_embed_structure)
        t_neg_data_description.append(t_neg_embed_description)
        t_neg_data_image.append(t_neg_embed_image)
        t_neg_data_multi.append(t_neg_embed_multi)

        h_neg = sample_negative_head(triples_set, entity_list, text_triple)[0]
        #print("negative head: ", h_neg)

        h_neg_embed_structure = entity_embedding_structure[h_neg]
        h_neg_embed_description = entity_embedding_description[h_neg]
        h_neg_embed_image = entity_embedding_image[h_neg]
        h_neg_embed_multi = entity_embedding_multi[h_neg]
        h_neg_data_structure.append(h_neg_embed_structure)
        h_neg_data_description.append(h_neg_embed_description)
        h_neg_data_image.append(h_neg_embed_image)
        h_neg_data_multi.append(h_neg_embed_multi)


    return np.asarray(h_data_structure), np.asarray(h_data_description), np.asarray(h_data_image), np.asarray(r_data), \
           np.asarray(t_data_structure), np.asarray(t_data_description), np.asarray(t_data_image), \
           np.asarray(t_neg_data_structure), np.asarray(t_neg_data_description), np.asarray(t_neg_data_image), \
           np.asarray(h_neg_data_structure), np.asarray(h_neg_data_description), np.asarray(h_neg_data_image), \
           np.asarray(h_data_multi), np.asarray(t_data_multi), np.asarray(h_neg_data_multi), np.asarray(t_neg_data_multi)
