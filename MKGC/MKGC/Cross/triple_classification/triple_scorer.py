...
import numpy as np
from MKGC.Cross.triple_classification import helper_functions as u
...
def calculate_triple_score(triple, rel_embd_dict, entity_structure_embd_dict, entity_description_embd_dict,
                           entity_image_embd_dict, entity_multi_embed_dict):
    head = triple[0]
    tail = triple[1]
    relation = triple[2]
    ...
    head_image_embedding = [entity_image_embd_dict[head]]
    head_multi_embedding = [entity_multi_embed_dict[head]]
    ...
    tail_image_embedding = [entity_image_embd_dict[tail]]
    tail_multi_embedding = [entity_multi_embed_dict[tail]]
    ...
    predictions1 = energy_fusion_pos.eval(feed_dict={r_input: rel_embedding,
                                                     ...
                                                     h_pos_image_input: head_image_embedding,
                                                     t_pos_image_input: tail_image_embedding,
                                                     h_pos_multi_input: head_multi_embedding,
                                                     t_pos_multi_input: tail_multi_embedding,
                                                     keep_prob: 1.0})
...
entity_image_embeddings = u.load_binary_file(p.image_entity_embeddings_file, 3)
entity_multi_embeddings = u.load_binary_file(p.multimodal_entity_embeddings_file, 3)
        ...
        for mode in modes:
            ...
            for triple in all_test_triples:
                score = calculate_triple_score(triple, relation_embeddings, entity_structure_embeddings,
                                               entity_description_embeddings, entity_image_embeddings,
                                               entity_multi_embeddings)
