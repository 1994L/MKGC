...
def load_entity_list(triple_file, entity_embeddings_structure, entity_embeddings_image):
    entity_list = []
...
def load_freebase_triple_data_multimodal(train_triples_file, entity_embeddings_structural,
    ...
    for triple in training_triples:
        ...
        if head in entity_embeddings_structural and tail in entity_embeddings_structural and \
            ...
            train_instance = (head_embd_structural, rel_embd, tail_embd_structural, head_embd_image, tail_embd_image,
                              head, rel, tail)
...
def get_batch_with_neg_tails_multimodal_top_k(training_data, triples_set, entity_list, start, end, entity_embedding_txt,entity_embedding_img,nr_neg_tails):
    h_data_structural = [] # head text embeddings
    h_data_img = [] # head image embeddings
    r_data = []  # relation embeddings
    t_data_structural = [] # tail text embeddings
    t_data_img = [] # tail image embeddings
    t_neg_data_structural = [] # negative tail text embeddings
    t_neg_data_img = [] # negative tail image embeddings
    ...
    for triple in batch_data:
        ...
        for i in range(nr_neg_tails):
            h_data_structural.append(triple[0])
            r_data.append(triple[1])
            t_data_structural.append(triple[2])
            h_data_img.append(triple[3])
            t_data_img.append(triple[4])
            ...
            t_neg = sample_negative_tail(triples_set, entity_list, text_triple)[1]
            t_neg_embed_structural = entity_embedding_txt[t_neg]
            t_neg_embed_img = entity_embedding_img[t_neg]

            t_neg_data_structural.append(t_neg_embed_structural)
            t_neg_data_img.append(t_neg_embed_img)


    return np.asarray(h_data_structural), np.asarray(h_data_img), np.asarray(r_data), np.asarray(t_data_structural),\
           np.asarray(t_data_img), np.asarray(t_neg_data_structural), np.asarray(t_neg_data_img)


def get_batch_with_neg_heads_and_neg_tails_multimodal(training_data, triples_set, entity_list, start, end,
                                                      entity_embedding_structure, entity_embedding_image):
    ...
    for triple in batch_data:
        ...
        h_data_image.append(triple[3])
        t_data_image.append(triple[4])

        text_triple = (triple[5], triple[6], triple[7])
    ...
    return np.asarray(h_data_structure), np.asarray(h_data_image), np.asarray(r_data), \
           np.asarray(t_data_structure), np.asarray(t_data_image), \
           np.asarray(t_neg_data_structure), np.asarray(t_neg_data_image), \
           np.asarray(h_neg_data_structure), np.asarray(h_neg_data_image)





def get_batch_with_neg_tails_hard_neg_top_k_multimodal(training_data, triples_set, entity_list, start, end,
                                                       entity_embedding_txt, entity_embedding_img, h_r_t_pos, r_input,
                                                       h_pos_txt_input, t_pos_txt_input, h_pos_img_input,
                                                       t_pos_img_input, keep_prob, sess, k):
