...
import MKGC.Add_des_str.util as u
...
training_data = u.load_freebase_triple_data_multimodal(param.train_triples_file, entity_embeddings_structural,
                                                       entity_embeddings_description, relation_embeddings)
...
with tf.name_scope('cosine'):
    ...
    h_fusion_mapped_pos = h_pos_structure_mapped + h_pos_description_mapped
    h_fusion_mapped_neg = h_neg_structure_mapped + h_neg_description_mapped

    t_fusion_mapped_pos = t_pos_structure_mapped + t_pos_description_mapped
    t_fusion_mapped_neg = t_neg_structure_mapped + t_neg_description_mapped
    ...
    kbc_loss1 = tf.maximum(0., param.margin - energy_fusion_neg + energy_fusion_pos)
    kbc_loss2 = tf.maximum(0., param.margin - score_fusion_neg + score_fusion_pos)
...
h_data_valid_structure, h_data_valid_description, r_data_valid, \
t_data_valid_structure, t_data_valid_description, t_neg_data_valid_structure, t_neg_data_valid_description, \
h_neg_data_valid_stucture, h_neg_data_valid_description = \
    ...
    for epoch in range(param.training_epochs):
       ...
       # training_data2 = training_data[:len(training_data)//3]
        ...
        for i in range(total_batch):
            ...
            h_data_structure, h_data_description, r_data, t_data_structure, \
            t_data_description, t_neg_data_structure, t_neg_data_description, \
            h_neg_data_structure, h_neg_data_description = u.get_batch_with_neg_heads_and_neg_tails_multimodal(
                training_data, triples_set, entity_list, start,
                end, entity_embeddings_structural, entity_embeddings_description)
