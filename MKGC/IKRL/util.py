...
def load_freebase_triple_data_multimodal(train_triples_file, entity_embeddings_txt, relation_embeddings):
    training_intances = []
    ...
    for triple in training_triples:
        ...
        if head in entity_embeddings_txt and tail in entity_embeddings_txt:
            ...
            train_instance = (head_embd_txt, rel_embd, tail_embd_txt, head, rel, tail)
...
def get_batch_with_neg_heads_and_neg_tails_relation_multimodal(training_data, triples_set, entity_list, start, end,
                                                               entity_embedding_txt, relation_embeddings):
   ...
   # print("chance",chance)
    ...
    for triple in batch_data:
        ...
        text_triple = (triple[3], triple[4], triple[5])
        # print("org triple: ", text_triple)
        ...
        counter += 1
    return np.asarray(h_data_txt), np.asarray(r_data), np.asarray(t_data_txt), \
           np.asarray(t_neg_data_txt), np.asarray(h_neg_data_txt),  np.asarray(r_neg_data)
